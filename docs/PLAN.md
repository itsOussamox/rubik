...
## 8. Graphics viewer prototype
- Optional OpenGL viewer compiled with `make GRAPHICS=1` (requires GLFW + OpenGL/GLU libraries on the host).
- Launch at runtime with `rubik -g "<scramble>"`; omit the scramble to only inspect the cube.
- When built without graphics support, the `-g` flag falls back to an instructive error message.
- Viewer currently renders a rotating solved cube to validate rendering pipeline before wiring cube state data.
